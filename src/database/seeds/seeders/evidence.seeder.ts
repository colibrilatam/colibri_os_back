...
import { MicroActionInstance } from 'src/micro-action-instance/entities/micro-action-instance.entity';
...
export async function seedEvidences(
  ...
  for (const mad of mainMads) {
    ...
    const savedEvidence = await evidenceRepo.save({
      ...
      submittedAt: isPending
        ? undefined
        : daysAgo(tramoIndex === 1 ? 95 : tramoIndex === 2 ? 65 : 12),
      approvedAt: isPending
        ? undefined
        : daysAgo(tramoIndex === 1 ? 93 : tramoIndex === 2 ? 63 : 10),
    } as Evidence);
  ...
  console.log(
    `✅ Evidencias creadas: ${savedEvidences.length} (${completas} completas, ${pendientes} pendiente)`,
  );
