...
export class HierarchyRepository {
  ...
  async findFullHierarchy(query: IQueryHierarchy): Promise<Tramo[]> {
    const { tramoId, onlyActive = true, includeResources = true } = query;
    ...
    const queryBuilder = this.tramoRepo
      ...
      .leftJoinAndSelect('category.pacs', 'pac', onlyActive ? 'pac.isActive = :active' : '1=1', {
        active: true,
      })
      .leftJoinAndSelect(
        'pac.microActionDefinitions',
        'mad',
        onlyActive ? '(mad.validTo IS NULL OR mad.validTo > NOW())' : '1=1',
      );
    ...
    if (includeResources) {
      queryBuilder.addOrderBy('pacLr.sortOrder', 'ASC').addOrderBy('madLr.sortOrder', 'ASC');
    }
