...
export class MecenasNftPortfolioRepository {
    ...
    async createMecenasNft(data: ICreateMecenasNft) {
        const mecenasNft = this.mecenasNftPortfolioRepository.create({
            mecenasUserId: data.mecenasUserId,
            nftProjectId: data.nftProjectId,
    ...
    async findByMecenasId(mecenasUserId: string) {
        return await this.mecenasNftPortfolioRepository.find({ where: { mecenasUserId }, relations: ['nftProject', 'targetProject'] })
    }

    
    async findAvailableByMecenasId(mecenasUserId: string) {
        return await this.mecenasNftPortfolioRepository.find({
            where: { mecenasUserId, targetProjectId: IsNull() },
            relations: ['nftProject'],
        });
    }
    async findByNftProjectId(nftProjectId: string) {
        return await this.mecenasNftPortfolioRepository.findOne({ where: { nftProjectId }, relations: ['nftProject'] })
    }
    ...
    async findById(id: string) {
        return await this.mecenasNftPortfolioRepository.findOne({ where: { id }, relations: ['nftProject'] })
    }
}
