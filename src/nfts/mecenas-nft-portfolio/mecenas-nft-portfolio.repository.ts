...
export class MecenasNftPortfolioRepository {
  constructor(
    ...
    private readonly mecenasNftPortfolioRepository: Repository<MecenasNftPortfolio>,
  ) {}
  ...
  async findByMecenasId(mecenasUserId: string) {
    return await this.mecenasNftPortfolioRepository.find({
      where: { mecenasUserId },
      relations: ['nftProject', 'targetProject'],
    });
  }
  ...
  async findByNftProjectId(nftProjectId: string) {
    return await this.mecenasNftPortfolioRepository.findOne({
      where: { nftProjectId },
      relations: ['nftProject'],
    });
  }
