...
export class NftProjectService {
  ...
  async associateToProject(nftProjectId: string, projectId: string): Promise<NftProject> {
    const nft = await this.findOne(nftProjectId);

    if (nft.projectId) {
      throw new ConflictException(`Este NFT ya está asociado al proyecto ${nft.projectId}`);
    }
